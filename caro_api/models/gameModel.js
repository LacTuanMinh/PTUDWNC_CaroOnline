...
module.exports = {
    ...
    getAllGamesByUserID: id => db.load(`SELECT ID, Name, Player1ID, Player2ID, Result, GameOverAt FROM Games WHERE (Player1ID = '${id}' OR Player2ID = '${id}') AND Status = 0 ORDER BY GameOverAt DESC`),

    checkGameExistByID: id => db.load(`SELECT ID,Password FROM Games WHERE ID = '${id}' AND (Status = 1 OR Status = 2)`),

    getGameStatusByID: id => db.load(`SELECT Status FROM Games WHERE ID = '${id}'`),
